...
export default function FreightPage() {
  ...
  // Push the calculated freight total to the bridge whenever it changes, so
  // the Prix de Revient page can pick it up with its "Reprendre le fret" button.
  React.useEffect(() => {
    saveFreightBridge({
      totalEur: totalFreight,
      departure,
      country: row.country,
      airport: row.airport,
    });
  }, [totalFreight, departure, row.country, row.airport]);
  ...
  return (
    <div>
      ...
      <p className="text-xs mt-6" style={{ color: "#8B978F" }}>
        Direction Côte d'Ivoire ?{" "}
        <Link to="/prix-de-revient" className="font-medium" style={{ color: "#3D5A73" }}>
          Calculez le prix de revient complet (droits, taxes, transitaire) →
        </Link>
      </p>
    </div>
  );
